...
DB_PATH =  os.path.join(PACKAGE_DIR, "signai_signs.db")
...
def run_on_image(image_path, model, engine, min_conf, language, strings):
    ...
    if img is None:
        return f"BŁĄD: Nie widzę pliku '{image_path}'."
    dets = detect_any(model, img, min_conf=min_conf)
    enriched = enrich(dets, language)
    out = draw(img, enriched)
    return handle_detections(enriched, engine, strings, verbose=True, notify_on_none=True)
